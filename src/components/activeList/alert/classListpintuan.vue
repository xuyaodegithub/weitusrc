  ...
  <div>
    <el-table
      ...
      <el-table-column
        label="成团人数">
        <template slot-scope="scope">
          <span>{{ scope.row.successValue }}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="已售数量"
      ...
      <el-table-column
        ...
        show-overflow-tooltip>
        <template slot-scope="scope">
          <span>{{ scope.row.limitBuyCount }}</span>
        </template>
      </el-table-column>
     <el-table-column
        label="审核状态"
    ...
    <div style="margin: 20px 0 0 10px;">
      <el-button type="primary" plain size="mini" @click="toggleSelection(productlistResult.rows)">批量选择</el-button>
...
//              }
          ...
          console.log(this.commodityResult.contents[this.addDataNumResult].dataList)
